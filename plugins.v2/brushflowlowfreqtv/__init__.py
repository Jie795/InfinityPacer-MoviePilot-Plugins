...
class BrushFlowLowFreqTV(_PluginBase):
    # region 全局定义

    # 插件名称
    plugin_name = "站点刷流（低频TV版）"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "brushflowlowfreqtv_"
    # 加载顺序
    plugin_order = 22
    # 可使用的用户级别
    auth_level = 2
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if brush_config.enabled:
            ...
            services.append({
                "id": "BrushFlowLowFreqTVCheck",
                "name": "站点刷流（低频TV版）检查服务",
                "trigger": "interval",
                "func": self.check,
            ...
            if torrent_content:
                ...
                if not state:
                    ...
                    if not torrent_hash:
                        ...
                        return None
                    # 成功添加到qBittorrent后，按要求调整文件优先级：
                    # 1) 将所有文件设置为不下载；2) 随机将部分文件设置为最高优先级
                    try:
                        self.__qb_set_random_file_priorities(torrent_hash=torrent_hash)
                    except Exception as e:
                        logger.error(f"设置qB文件优先级失败：{str(e)}")
                    return torrent_hash
            return None
    ...
    def __qb_set_random_file_priorities(self, torrent_hash: str):
        """
        在qBittorrent中将种子所有文件设为不下载，然后随机挑选少量文件设置为最高优先级。
        规则：
        - 如果文件数为2-4：仅选1个文件设为最高优先级
        - 如果文件数>=5：选2个文件设为最高优先级
        - 其他（如只有1个文件）：保持1个最高优先级
        注意：磁力链接需等待元数据加载后才有文件列表，这里做短轮询。
        """
        # 仅对qBittorrent生效
        if not self.downloader_helper.is_downloader("qbittorrent", service=self.service_info):
            return
        ...
        while waited < max_wait_seconds:
            try:
                files = downloader.qbc.torrents_files(torrent_hash=torrent_hash) or []
                if files:
                    break
            except Exception as e:
                logger.debug(f"获取qB文件列表失败，将重试：{str(e)}")
            time.sleep(interval)
            waited += interval

        if not files:
            logger.info("qB未能在限定时间内获取到文件列表，跳过优先级设置")
            return

        total_files = len(files)
        # 全部设为不下载（0）
        try:
            all_ids = list(range(total_files))
            # 分批避免过长参数
            step = 1000
            for i in range(0, total_files, step):
                batch_ids = all_ids[i:i + step]
                downloader.qbc.torrents_file_priority(torrent_hash=torrent_hash,
                                                      file_ids=batch_ids,
                                                      priority=0)
        except Exception as e:
            logger.error(f"批量设置为不下载失败：{str(e)}")

        # 决定要提权的文件数量
        if total_files <= 1:
            promote_count = 1
        elif 2 <= total_files <= 4:
            promote_count = 1
        elif 5 <= total_files <= 9:
            promote_count = 2
        else:  # total_files >= 10
            promote_count = 3
        ...
        # 随机挑选不同的文件索引
        try:
            chosen_ids = random.sample(range(total_files), k=min(promote_count, total_files))
        except ValueError:
            chosen_ids = [0]
        ...
        try:
            downloader.qbc.torrents_file_priority(torrent_hash=torrent_hash,
                                                  file_ids=chosen_ids,
                                                  priority=7)
            logger.info(f"已将 {len(chosen_ids)}/{total_files} 个文件设置为最高优先级，hash={torrent_hash}")
        except Exception as e:
            logger.error(f"设置文件最高优先级失败：{str(e)}")
